...
**Доступ через Ingress:** в манифесте `k8s/ingress.yaml` добавлен путь **/pgadmin** к сервису pgadmin. Если у вас настроен Ingress (например, Yandex ALB) и приложение открывается по одному домену/хосту, pgAdmin будет доступен по тому же адресу: **https://&lt;ваш-хост&gt;/pgadmin/** (логин и пароль те же; для подключения к БД в pgAdmin укажите Host **`db`**). Образ pgAdmin в манифесте настроен с `PGADMIN_CONFIG_SCRIPT_NAME=/pgadmin` для работы по подпути. При деплое через CI/CD манифесты `pgadmin-deployment.yaml` и `ingress.yaml` применяются автоматически.

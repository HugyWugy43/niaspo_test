...
В результате в кластере всегда развёрнута **актуальная тестовая среда**.
...
### Способ 1: pgAdmin на том же IP, что и фронтенд (порт 5050)
...
pgAdmin в кластере доступен как внутренний сервис `pgadmin` и открывается **через тот же внешний IP, что и фронтенд**, но на отдельном порту **5050**:

- открыть: **http://<EXTERNAL-IP фронтенда>:5050/** (например: `http://158.160.189.212:5050/`)

- **Вход в pgAdmin:** Email настраивается вами в файле k8s, пароль тоже
